...
var fastingDetectionDebug = {
  ...
  detectFastingPeriods: function(dayData, nextDayData, options) {
    ...
    console.log('🔢 FASTING MODULE BUILD: FASTING-DEBUG-v3.11-FILTER-SYNTHETIC-CGM - 2026-01-28-13:45 EST');
        ...
        if (fastingDuration >= 2) {
          ...
          if (allCGMData.length > 0) {
            fastingCGMData = allCGMData.filter(function(cgm) {
              ...
              var isInTimeRange = cgmTime >= fastingStart.toDate() && cgmTime <= actualEndTime;
              
              // Exclude synthetic/fake readings (rawbg type with transparent color)
              // These are chart markers, not real CGM data
              var isSyntheticReading = cgm.type === 'rawbg' && cgm.color === 'transparent';
              
              return isInTimeRange && !isSyntheticReading;
            });
          }
          
          console.log('📊 CGM data for fasting period:', fastingCGMData.length, 'readings from', allCGMData.length, 'total readings');
          console.log('🕐 Fasting period time range:', fastingStart.format('YYYY-MM-DD HH:mm'), 'to', moment(actualEndTime).format('YYYY-MM-DD HH:mm'));
          
          if (fastingCGMData.length > 0) {
            console.log('🔍 First fasting CGM reading:', fastingCGMData[0], 'time:', moment(fastingCGMData[0].date || fastingCGMData[0].mills).format('YYYY-MM-DD HH:mm'));
            console.log('🔍 Last fasting CGM reading:', fastingCGMData[fastingCGMData.length - 1], 'time:', moment(fastingCGMData[fastingCGMData.length - 1].date || fastingCGMData[fastingCGMData.length - 1].mills).format('YYYY-MM-DD HH:mm'));
          }
          ...
          if (fastingCGMData.length > 0) {
            ...
            if (glucoseValues.length > 0) {
              ...
              console.log('🔍 Raw glucose values sample (first 10):', glucoseValues.slice(0, 10));
              console.log('🔍 Calculated min:', min, 'max:', max, 'range:', range);
              console.log('🔍 Total glucose values:', glucoseValues.length);
              ...
              // options parameter contains targetLow and targetHigh passed from daytoday.js
              // These are already in mg/dL from client.settings.thresholds
              var targetLow = options.targetLow;
              var targetHigh = options.targetHigh;
              
              if (!targetLow || !targetHigh || targetLow < 40 || targetHigh > 400) {
                console.warn('⚠️ Invalid target range detected:', targetLow, '-', targetHigh, '- using defaults');
                targetLow = 70;
                targetHigh = 180;
              }
              
              // Get units from options (passed from daytoday.js)
              var displayUnits = options.units || 'mg/dl';
              ...
              // Convert target range for display if needed
              var displayTargetLow = targetLow;
              var displayTargetHigh = targetHigh;
              if (displayUnits === 'mmol') {
                // Convert mg/dL to mmol/L (divide by 18)
                displayTargetLow = Math.round((targetLow / 18) * 10) / 10;
                displayTargetHigh = Math.round((targetHigh / 18) * 10) / 10;
              }
              ...
              glucoseStats = {
                ...
                targetRange: displayTargetLow + '-' + displayTargetHigh + ' ' + unitsLabel,
                trend: trend,
                rateOfChange: Math.round(rateOfChange * 10) / 10,
                qualityScore: qualityScore,
                units: displayUnits  // Store units for later use
              };
              ...
              console.log('   Min:', glucoseStats.min, 'Max:', glucoseStats.max);
          ...
          // Collect treatment data during fasting period for insulin delivery analysis
          // This includes temp basals, SMBs, and any other insulin delivery
          var allTreatments = (dayData.treatments || []).slice();
          if (nextDayData && nextDayData.treatments) {
            allTreatments = allTreatments.concat(nextDayData.treatments);
          }
          
          var fastingTreatments = allTreatments.filter(function(treatment) {
            var treatmentTime = new Date(treatment.mills || treatment.timestamp || treatment.created_at);
            return treatmentTime >= fastingStart.toDate() && treatmentTime <= actualEndTime;
          });
          
          console.log('💉 Treatments during fasting period:', fastingTreatments.length);
          if (fastingTreatments.length > 0) {
            console.log('💉 Sample treatments (first 3):', fastingTreatments.slice(0, 3));
          }
          ...
          var realPeriod = {
            ...
            treatments: fastingTreatments  // Add treatments for insulin delivery analysis
          };
          
          console.log('🎯 REAL fasting period created with', glucoseReadings.length, 'glucose readings and', fastingTreatments.length, 'treatments');
          console.log('🔍 Sample glucose readings (first 3):', glucoseReadings.slice(0, 3));
