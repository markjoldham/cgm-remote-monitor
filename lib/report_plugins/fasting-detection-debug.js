...
var fastingDetectionDebug = {
   ...
   */
  detectFastingPeriods: function(dayData, nextDayData) {
    console.log('=== FASTING DETECTION DEBUG ===');
    console.log('🔢 FASTING MODULE BUILD: FASTING-DEBUG-v3.3-FIX-DAY-ORDER - 2026-01-28-11:30 EST');
    ...
    if (dayData.treatments && dayData.treatments.length > 0) {
      ...
      if (mealTreatments.length > 0) {
        ...
        // If no next meal found, use end of next day (or end of current day if no next day)
        var fastingEndTime = nextMealTime || (nextDayData ? moment(nextDayData.date).endOf('day') : endOfDay);
        ...
        console.log('🕐 Fasting end time:', fastingEndTime.format('YYYY-MM-DD HH:mm'), nextMealTime ? '(next meal)' : '(end of data)');
          ...
          if (isCurrentDay) {
            ...
            if (allCGMData.length > 0) {
              ...
              allCGMData.forEach(function(cgm) {
                ...
                // Only consider CGM readings after fasting start
                if (cgmTime.isAfter(fastingStart)) {
