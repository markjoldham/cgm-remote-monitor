...
var fastingDetection = {
  ...
  detectFastingPeriods: function(dayData, options) {
    ...
    var config = {
      minFastingDuration: options.minFastingDuration || 3, // hours
      maxFastingDuration: options.maxFastingDuration || 12, // hours
      carbThreshold: options.carbThreshold || 5, // grams - ignore small carb corrections
      bolusThreshold: options.bolusThreshold || 0.5, // units - ignore small corrections
      iobThreshold: options.iobThreshold || 1.0, // units - max IOB to consider fasting
      glucoseStabilityThreshold: options.glucoseStabilityThreshold || 30, // mg/dL variation
  ...
  checkIfBreaksFasting: function(treatments, config) {
    return treatments.some(function(t) {
      ...
      // Significant bolus (meal bolus, not correction)
      if (t.insulin && t.insulin > config.bolusThreshold && t.carbs) return true;
  ...
  validateFastingPeriod: function(period, config) {
    ...
    // Minimum data points
    if (period.cgmReadings < 6) return false; // At least 6 readings (30 min of data)
    
    // Glucose stability check (optional - might be too restrictive)
