...
daytoday.html = function html (client) {
  ...
  var ret =
    ...
    `<label><input type="checkbox" id="rp_optionsfasting" ${reportStorage.getValue('fasting') ? "checked" : ""}><span style="color:#9932cc;opacity:0.8">${translate('Fasting periods')}</span></label>` +
    '&nbsp;' + translate('Size') +
    ' <select id="rp_size">' +
...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  console.log('🚨 DAYTODAY REPORT STARTING - FASTING DEBUG VERSION 🚨');
  console.log('🔢 BUILD: FASTING-DEBUG-v3.11-FILTER-SYNTHETIC-CGM - 2026-01-28-13:45 EST');
  console.log('This message should appear if the new code is loaded');
  ...
  console.log('=== DAYTODAY REPORT DEBUG ===');
  console.log('options object:', options);
  console.log('options.fasting:', options.fasting);
  console.log('report_plugins.fastingDetection available:', !!report_plugins.fastingDetection);
  
  var allFastingPeriods = [];
  
  // ALWAYS try to run fasting detection for debugging, regardless of checkbox
  console.log('🔍 FORCING FASTING DETECTION FOR DEBUG...');
  var fastingDetection = report_plugins.fastingDetection;
  
  if (fastingDetection) {
    console.log('✅ Fasting detection module loaded successfully');
    
    // Get the user's target range from client settings (always in mg/dL)
    var userTargetLow = client.settings.thresholds.bgTargetBottom;
    var userTargetHigh = client.settings.thresholds.bgTargetTop;
    
    console.log('🎯 User target range from settings:', userTargetLow, '-', userTargetHigh, 'mg/dL');
    
    sorteddaystoshow.forEach(function(day, index) {
      var dayData = {
        date: day,
        treatments: datastorage[day].treatments || [],
        cgmData: datastorage[day].sgv || []
      };
      
      // Get next day's data for rolling fasting periods across midnight
      // NOTE: sorteddaystoshow is in DESCENDING order (newest first)
      // So we need to look BACKWARD in the array (index - 1) to get tomorrow
      var nextDay = sorteddaystoshow[index - 1];
      var nextDayData = null;
      if (nextDay && datastorage[nextDay]) {
        nextDayData = {
          date: nextDay,
          treatments: datastorage[nextDay].treatments || [],
          cgmData: datastorage[nextDay].sgv || []
        };
      }
      
      // Create options object with user's target range and units
      var fastingOptions = {
        targetLow: userTargetLow,
        targetHigh: userTargetHigh,
        units: client.settings.units || 'mg/dl'
      };
      
      console.log('📊 Processing day for fasting detection:', day, {
        treatments: dayData.treatments.length,
        cgmData: dayData.cgmData.length,
        hasNextDay: !!nextDayData,
        nextDayDate: nextDay || 'none',
        targetLow: fastingOptions.targetLow,
        targetHigh: fastingOptions.targetHigh,
        sampleTreatment: dayData.treatments.length > 0 ? dayData.treatments[0] : 'none',
        sampleCGM: dayData.cgmData.length > 0 ? dayData.cgmData[0] : 'none'
      });
      
      var fastingPeriods = fastingDetection.detectFastingPeriods(dayData, nextDayData, fastingOptions);
      allFastingPeriods = allFastingPeriods.concat(fastingPeriods.map(function(period) {
        period.day = day;
        return period;
      }));
    });
    
    console.log('🎯 Total fasting periods found across all days:', allFastingPeriods.length);
  } else {
    console.error('❌ Fasting detection module NOT AVAILABLE');
    console.log('report_plugins object:', report_plugins);
    console.log('Available report_plugins properties:', Object.keys(report_plugins || {}));
  }
  console.log('=== END DAYTODAY REPORT DEBUG ===');
  ...
  // Display fasting periods summary
  if (options.fasting && allFastingPeriods.length > 0) {
    var fastingHtml = '<br><br><div style="border: 1px solid #9932cc; padding: 10px; margin: 10px 0; background-color: #f9f9ff;">';
    fastingHtml += '<h3 style="color: #9932cc; margin-top: 0;">' + translate('Fasting Periods Analysis') + '</h3>';
    
    // Summary statistics
    var excellentPeriods = allFastingPeriods.filter(function(p) { return p.quality === 'excellent'; }).length;
    var goodPeriods = allFastingPeriods.filter(function(p) { return p.quality === 'good'; }).length;
    var totalHours = allFastingPeriods.reduce(function(sum, p) { return sum + p.duration; }, 0);
    
    fastingHtml += '<p><b>' + translate('Total fasting periods found') + ':</b> ' + allFastingPeriods.length + ' ';
    fastingHtml += '(<span style="color: #008000;">' + excellentPeriods + ' excellent</span>, ';
    fastingHtml += '<span style="color: #0066cc;">' + goodPeriods + ' good</span>)</p>';
    fastingHtml += '<p><b>' + translate('Total fasting time') + ':</b> ' + totalHours.toFixed(1) + ' hours</p>';
    
    // Best periods for basal testing - show all excellent periods, then good ones if needed
    var excellentPeriods = allFastingPeriods.filter(function(p) { 
      return p.quality === 'excellent'; 
    });
    var goodPeriods = allFastingPeriods.filter(function(p) { 
      return p.quality === 'good'; 
    });
    
    // Show all excellent periods, plus good periods if we have fewer than 3 total
    var bestPeriods = excellentPeriods.slice();
    if (bestPeriods.length < 3) {
      bestPeriods = bestPeriods.concat(goodPeriods.slice(0, 3 - bestPeriods.length));
    }
    
    if (bestPeriods.length > 0) {
      var sectionTitle = excellentPeriods.length > 0 ? 
        'All excellent periods for basal rate analysis' + (excellentPeriods.length > 3 ? ' (' + excellentPeriods.length + ' found)' : '') :
        'Best periods for basal rate analysis';
      fastingHtml += '<p><b>' + translate(sectionTitle) + ':</b></p>';
      fastingHtml += '<ul>';
      bestPeriods.forEach(function(period) {
        var startTime = moment(period.startTime).format('MMM DD HH:mm');
        var endTime = moment(period.endTime).format('HH:mm');
        var qualityColor = period.quality === 'excellent' ? '#008000' : '#0066cc';
        
        // Get display units
        var displayUnits = client.settings.units || 'mg/dl';
        var unitsLabel = displayUnits === 'mmol' ? 'mmol/L' : 'mg/dL';
        
        fastingHtml += '<li>';
        fastingHtml += '<b>' + startTime + ' - ' + endTime + '</b> ';
        fastingHtml += '(' + period.duration.toFixed(1) + 'h, ' + period.periodType + ') - ';
        fastingHtml += '<span style="color: ' + qualityColor + ';">' + period.quality + '</span>';
        
        if (period.glucoseStats) {
          // Convert glucose values for display if needed
          var displayMean = period.glucoseStats.mean;
          var displayRange = period.glucoseStats.range;
          var displayRateOfChange = period.glucoseStats.rateOfChange;
          
          if (displayUnits === 'mmol') {
            displayMean = Math.round((period.glucoseStats.mean / 18) * 10) / 10;
            displayRange = Math.round((period.glucoseStats.range / 18) * 10) / 10;
            displayRateOfChange = Math.round((period.glucoseStats.rateOfChange / 18) * 10) / 10;
          }
          
          fastingHtml += '<br><small>';
          fastingHtml += 'Avg BG: ' + displayMean + ' ' + unitsLabel + ', ';
          fastingHtml += 'Range: ' + displayRange + ' ' + unitsLabel + ', ';
          fastingHtml += 'CV: ' + period.glucoseStats.cv + '% ';
          
          // Add CV quality indicator
          if (period.glucoseStats.cv < 20) {
            fastingHtml += '<span style="color: #008000;">(Excellent)</span>';
          } else if (period.glucoseStats.cv < 30) {
            fastingHtml += '<span style="color: #0066cc;">(Good)</span>';
          } else if (period.glucoseStats.cv < 40) {
            fastingHtml += '<span style="color: #ff8c00;">(Fair)</span>';
          } else {
            fastingHtml += '<span style="color: #cc0000;">(Poor)</span>';
          }
          
          fastingHtml += '<br>';
          fastingHtml += 'Time in Range: ' + period.glucoseStats.timeInRange + '%';
          if (period.glucoseStats.targetRange) {
            fastingHtml += ' (' + period.glucoseStats.targetRange + ')';
          }
          fastingHtml += ', Trend: ' + period.glucoseStats.trend;
          
          if (period.glucoseStats.trend !== 'stable') {
            fastingHtml += ' (' + (period.glucoseStats.rateOfChange > 0 ? '+' : '') + displayRateOfChange + ' ' + unitsLabel + '/hr)';
          }
          
          fastingHtml += '<br>';
          fastingHtml += 'Quality Score: ' + period.glucoseStats.qualityScore + '/100 for basal testing';
          fastingHtml += '</small>';
        }
        fastingHtml += '</li>';
      });
    ...
    fastingHtml += '<p><small><i>' + translate('Fasting periods are identified automatically based on absence of carbs, minimal bolus insulin, and glucose stability. Use these periods to evaluate basal rate effectiveness.') + '</i></small></p>';
    
    // Add glucose stability metrics explanation
    fastingHtml += '<hr style="margin: 10px 0;">';
    fastingHtml += '<h4 style="color: #9932cc; margin: 10px 0;">Understanding Glucose Stability Metrics</h4>';
    fastingHtml += '<table style="width: 100%; font-size: 11px; border-collapse: collapse;">';
    fastingHtml += '<tr style="background-color: #f0f0f0;">';
    fastingHtml += '<th style="padding: 5px; text-align: left; border: 1px solid #ddd;">Metric</th>';
    fastingHtml += '<th style="padding: 5px; text-align: left; border: 1px solid #ddd;">Description</th>';
    fastingHtml += '<th style="padding: 5px; text-align: left; border: 1px solid #ddd;">Ideal for Basal Testing</th>';
    fastingHtml += '</tr>';
    
    fastingHtml += '<tr>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;"><b>CV (Coefficient of Variation)</b></td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;">Standard measure of glucose variability. Lower is better.</td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;"><span style="color: #008000;">Excellent: &lt;20%</span>, <span style="color: #0066cc;">Good: 20-30%</span>, <span style="color: #ff8c00;">Fair: 30-40%</span></td>';
    fastingHtml += '</tr>';
    
    fastingHtml += '<tr style="background-color: #f9f9f9;">';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;"><b>Time in Range</b></td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;">Percentage of readings within your configured target range.</td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;">&gt;80% is ideal for basal testing</td>';
    fastingHtml += '</tr>';
    
    fastingHtml += '<tr>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;"><b>Trend</b></td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;">Overall glucose direction during the period.</td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;"><b>Stable</b> is best. Rising/falling may indicate basal adjustment needed.</td>';
    fastingHtml += '</tr>';
    
    fastingHtml += '<tr style="background-color: #f9f9f9;">';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;"><b>Quality Score</b></td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;">Overall suitability for basal rate analysis (0-100).</td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;">&gt;70 is excellent, 50-70 is good, &lt;50 may be less reliable</td>';
    fastingHtml += '</tr>';
    
    fastingHtml += '<tr>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;"><b>Range</b></td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;">Difference between highest and lowest glucose.</td>';
    fastingHtml += '<td style="padding: 5px; border: 1px solid #ddd;">&lt;30 mg/dL is excellent, &lt;50 mg/dL is acceptable</td>';
    fastingHtml += '</tr>';
    
    fastingHtml += '</table>';
    
    fastingHtml += '<p style="margin-top: 10px; font-size: 11px;"><b>Tip:</b> For accurate basal rate testing, choose periods with low CV (&lt;20%), high time in range (&gt;80%), and stable trends. Overnight periods (6+ hours) are typically most reliable.</p>';
    
    fastingHtml += '</div>';
    
    $('#daytodaycharts').append(fastingHtml);
    
    // Task 11.1: Call basal rate optimizer after fasting periods summary
    // Requirement 8.1: Display recommendations section after fasting periods summary
    if (report_plugins.basalRateOptimizer && allFastingPeriods.length > 0) {
      console.log('Calling basal rate optimizer with', allFastingPeriods.length, 'fasting periods');
      
      // Read profile data for personalized recommendations
      var profileData = report_plugins.basalRateOptimizer.readProfileData(profile);
      
      // Add TDD data from actual insulin delivery
      profileData.tdd = tddAverage;
      profileData.tddDays = datastorage.alldays;
      profileData.units = client.settings.units || 'mg/dl';  // Add units for display
      console.log('💉 Passing TDD to basal rate optimizer:', tddAverage.toFixed(1), 'U/day from', datastorage.alldays, 'days');
      console.log('📊 Display units:', profileData.units);
      
      // Generate recommendations
      var recommendationsResult = report_plugins.basalRateOptimizer.generateRecommendations(
        allFastingPeriods,
        profileData
      );
      
      if (recommendationsResult && !recommendationsResult.error) {
        // Generate HTML for recommendations
        var recommendationsHtml = report_plugins.basalRateOptimizer.generateRecommendationsHTML(
          recommendationsResult.recommendations,
          recommendationsResult.metadata,
          profileData  // Pass profileData for units
        );
        
        // Add CSS styles for recommendations display
        var stylesHtml = '<style>' +
          '.basal-rate-recommendations { margin: 20px 0; padding: 15px; border: 2px solid #0066cc; background-color: #f0f8ff; }' +
          '.basal-rate-recommendations h3 { color: #0066cc; margin-top: 0; }' +
          '.disclaimer { padding: 10px; margin: 10px 0; background-color: #fff3cd; border: 2px solid #ffc107; border-radius: 5px; }' +
          '.warning-box { font-size: 14px; }' +
          '.analysis-summary { padding: 10px; margin: 10px 0; background-color: #e7f3ff; border-left: 4px solid #0066cc; }' +
          '.time-blocks { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 15px; margin: 15px 0; }' +
          '.basal-optimizer-time-block { padding: 15px; border: 2px solid #ddd; border-radius: 8px; background-color: white; }' +
          '.basal-optimizer-time-block h4 { margin-top: 0; color: #333; }' +
          '.confidence-badge { display: inline-block; padding: 5px 10px; border-radius: 4px; font-weight: bold; font-size: 12px; margin-bottom: 10px; }' +
          '.confidence-green { background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; }' +
          '.confidence-yellow { background-color: #fff3cd; color: #856404; border: 1px solid #ffeaa7; }' +
          '.confidence-red { background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; }' +
          '.basal-rates { margin: 10px 0; }' +
          '.rate-item { margin: 5px 0; font-size: 14px; }' +
          '.rate-label { font-weight: bold; }' +
          '.rate-value { color: #0066cc; font-weight: bold; }' +
          '.rate-value.adjustment { color: #ff6600; }' +
          '.rate-value.new-rate { color: #009900; }' +
          '.recommendation-text { margin: 10px 0; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #0066cc; font-size: 14px; }' +
          '.reasoning { margin: 10px 0; font-size: 13px; color: #555; }' +
          '.supporting-data { margin: 10px 0; font-size: 12px; }' +
          '.supporting-data table { width: 100%; border-collapse: collapse; margin-top: 5px; }' +
          '.supporting-data th, .supporting-data td { padding: 5px; border: 1px solid #ddd; text-align: left; }' +
          '.supporting-data th { background-color: #f0f0f0; font-weight: bold; }' +
          '.message { margin: 10px 0; padding: 10px; background-color: #fff3cd; border-left: 4px solid #ffc107; font-style: italic; }' +
          '.warnings-section { margin: 10px 0; }' +
          '.warning-message { padding: 8px; margin: 5px 0; background-color: #fff3cd; border-left: 4px solid #ffc107; font-size: 12px; }' +
          '.safety-tips { margin: 20px 0; padding: 15px; background-color: #e7f3ff; border-left: 4px solid #0066cc; }' +
          '.safety-tips h4 { margin-top: 0; color: #0066cc; }' +
          '.safety-tips ul { margin: 10px 0; padding-left: 20px; }' +
          '.safety-tips li { margin: 5px 0; }' +
          '.export-buttons { margin: 15px 0; text-align: center; }' +
          '.export-button { padding: 10px 20px; margin: 5px; background-color: #0066cc; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 14px; font-weight: bold; }' +
          '.export-button:hover { background-color: #0052a3; }' +
          '.export-button:active { background-color: #003d7a; }' +
          '</style>';
        
        $('#daytodaycharts').append(stylesHtml + recommendationsHtml);
        
        // Store recommendations data globally for export functions
        window.basalRateRecommendationsData = {
          recommendations: recommendationsResult.recommendations,
          metadata: recommendationsResult.metadata
        };
        
        // Task 11.2: Add export button handlers (Requirements 9.1, 9.2)
        // Define global export functions for CSV export
        window.exportBasalRecommendationsToCSV = function() {
          console.log('Exporting basal rate recommendations to CSV...');
          
          if (!window.basalRateRecommendationsData) {
            alert('No recommendations data available for export');
            return;
          }
          
          try {
            var csv = report_plugins.basalRateOptimizer.exportToCSV(
              window.basalRateRecommendationsData.recommendations,
              window.basalRateRecommendationsData.metadata,
              profileData  // Pass profileData for units
            );
            ...
            if (link.download !== undefined) {
              ...
              var filename = 'basal-rate-recommendations-' + 
                window.basalRateRecommendationsData.metadata.startDate + '-to-' + 
                window.basalRateRecommendationsData.metadata.endDate + '.csv';
        ...
        // Define global export function for PDF export
        window.exportBasalRecommendationsToPDF = function() {
          console.log('Exporting basal rate recommendations to PDF...');
          ...
          try {
            ...
            var printHTML = report_plugins.basalRateOptimizer.generatePrintHTML(
              window.basalRateRecommendationsData.recommendations,
              window.basalRateRecommendationsData.metadata,
              profileData  // Pass profileData for units
            );
            ...
            printWindow.onload = function() {
              setTimeout(function() {
                printWindow.print();
                // Note: We don't close the window automatically because user might cancel print
              }, 250);
            };
        ...
        console.log('Basal rate recommendations displayed successfully');
      } else {
        console.log('Basal rate optimizer returned error:', recommendationsResult ? recommendationsResult.error : 'unknown');
      }
    } else if (!report_plugins.basalRateOptimizer) {
      console.log('Basal rate optimizer plugin not available');
    }
  }
  ...
  function drawChart (day, data, options) {
    ...
    // add fasting period overlays
    if (options.fasting && allFastingPeriods) {
      // Find fasting periods that overlap with this day (not just start on this day)
      var dayStart = moment(day).startOf('day');
      var dayEnd = moment(day).endOf('day');
      
      var dayFastingPeriods = allFastingPeriods.filter(function(period) {
        var periodStart = moment(period.startTime);
        var periodEnd = moment(period.endTime);
        
        // Include period if it overlaps with this day at all
        return periodEnd.isAfter(dayStart) && periodStart.isBefore(dayEnd);
      });
      
      dayFastingPeriods.forEach(function(period) {
        var startTime = new Date(period.startTime);
        var endTime = new Date(period.endTime);
        
        // Determine if this is a continuation from previous day or extends to next day
        var startsBeforeDay = moment(startTime).isBefore(dayStart);
        var endsAfterDay = moment(endTime).isAfter(dayEnd);
        
        // Only draw if period overlaps with chart data range
        if (endTime >= dataRange[0] && startTime <= dataRange[1]) {
          var rectStartX = Math.max(xScale2(startTime), xScale2(dataRange[0]));
          var rectEndX = Math.min(xScale2(endTime), xScale2(dataRange[1]));
          var rectWidth = rectEndX - rectStartX;
          
          if (rectWidth > 0) {
            // Color based on quality
            var fillColor = period.quality === 'excellent' ? '#E6E6FA' : 
                           period.quality === 'good' ? '#F0E6FF' : 
                           period.quality === 'fair' ? '#F5F0FF' : '#FAFAFA';
            var strokeColor = period.quality === 'excellent' ? '#9932CC' : 
                             period.quality === 'good' ? '#BA55D3' : 
                             period.quality === 'fair' ? '#DDA0DD' : '#D3D3D3';
            
            // Add fasting period background
            context.append('rect')
              .attr('x', rectStartX + padding.left)
              .attr('y', padding.top)
              ...
              .on('mouseover', function() {
                ...
                var tooltipText = '<b>Fasting Period</b>';
                
                // Get display units
                var displayUnits = client.settings.units || 'mg/dl';
                var unitsLabel = displayUnits === 'mmol' ? 'mmol/L' : 'mg/dL';
                
                // Indicate if this is a continuation or extension
                if (startsBeforeDay && endsAfterDay) {
                  tooltipText += ' <i>(continues from previous day and into next)</i>';
                } else if (startsBeforeDay) {
                  tooltipText += ' <i>(continued from previous day)</i>';
                } else if (endsAfterDay) {
                  tooltipText += ' <i>(continues into next day)</i>';
                }
                
                tooltipText += '<br/>';
                tooltipText += 'Total Duration: ' + period.duration.toFixed(1) + ' hours<br/>';
                tooltipText += 'Full Period: ' + moment(period.startTime).format('MMM DD HH:mm') + ' - ' + moment(period.endTime).format('MMM DD HH:mm') + '<br/>';
                tooltipText += 'Type: ' + period.periodType + '<br/>' +
                  'Quality: ' + period.quality + '<br/>';
                if (period.glucoseStats) {
                  // Convert glucose values for display if needed
                  var displayMean = period.glucoseStats.mean;
                  var displayMin = period.glucoseStats.min;
                  var displayMax = period.glucoseStats.max;
                  var displayRange = period.glucoseStats.range;
                  var displayRateOfChange = period.glucoseStats.rateOfChange;
                  
                  if (displayUnits === 'mmol') {
                    displayMean = Math.round((period.glucoseStats.mean / 18) * 10) / 10;
                    displayMin = Math.round((period.glucoseStats.min / 18) * 10) / 10;
                    displayMax = Math.round((period.glucoseStats.max / 18) * 10) / 10;
                    displayRange = Math.round((period.glucoseStats.range / 18) * 10) / 10;
                    displayRateOfChange = Math.round((period.glucoseStats.rateOfChange / 18) * 10) / 10;
                  }
                  
                  tooltipText += '<hr style="margin: 5px 0;">';
                  tooltipText += '<b>Glucose Stability Analysis:</b><br/>';
                  tooltipText += 'Mean: ' + displayMean + ' ' + unitsLabel + '<br/>';
                  tooltipText += 'Glucose Range: ' + displayMin + '-' + displayMax + ' ' + unitsLabel + ' (span: ' + displayRange + ')<br/>';
                  tooltipText += 'CV: ' + period.glucoseStats.cv + '% ';
                  
                  // Add CV interpretation
                  if (period.glucoseStats.cv < 20) {
                    tooltipText += '(Excellent)<br/>';
                  } else if (period.glucoseStats.cv < 30) {
                    tooltipText += '(Good)<br/>';
                  } else if (period.glucoseStats.cv < 40) {
                    tooltipText += '(Fair)<br/>';
                  ...
                  tooltipText += 'Time in Range: ' + period.glucoseStats.timeInRange + '%';
                  if (period.glucoseStats.targetRange) {
                    tooltipText += ' (' + period.glucoseStats.targetRange + ')';
                  }
                  tooltipText += '<br/>';
                  tooltipText += 'Trend: ' + period.glucoseStats.trend;
                  
                  if (period.glucoseStats.trend !== 'stable') {
                    tooltipText += ' (' + (period.glucoseStats.rateOfChange > 0 ? '+' : '') + displayRateOfChange + ' ' + unitsLabel + '/hr)';
                  }
                  
                  tooltipText += '<br/>';
                  tooltipText += '<b>Basal Testing Score: ' + period.glucoseStats.qualityScore + '/100</b>';
                }
                client.tooltip.html(tooltipText)
            ...
            // Add label for high-quality periods (only on the day where it starts)
            if ((period.quality === 'excellent' || period.quality === 'good') && !startsBeforeDay) {
              var labelX = rectStartX + rectWidth / 2;
              var labelY = padding.top + 20;
              
              var labelText = 'FASTING (' + period.duration.toFixed(1) + 'h)';
              if (endsAfterDay) {
                labelText += ' →'; // Arrow to indicate continuation
              }
              
              context.append('text')
                .attr('x', labelX + padding.left)
                .attr('y', labelY)
            ...
            // Add continuation indicator for periods that started yesterday
            if (startsBeforeDay && (period.quality === 'excellent' || period.quality === 'good')) {
              var contLabelX = rectStartX + 30;
              var contLabelY = padding.top + 20;
              
              context.append('text')
                .attr('x', contLabelX + padding.left)
                .attr('y', contLabelY)
