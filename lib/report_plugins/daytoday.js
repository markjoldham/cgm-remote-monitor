...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  if (fastingDetection) {
    ...
    sorteddaystoshow.forEach(function(day, index) {
      ...
      // Create options object with user's target range
      var fastingOptions = {
    ...
    if (bestPeriods.length > 0) {
      ...
      bestPeriods.forEach(function(period) {
        ...
        if (period.glucoseStats) {
          ...
          fastingHtml += 'Avg BG: ' + period.glucoseStats.mean + ' mg/dL, ';
          fastingHtml += 'Range: ' + period.glucoseStats.range + ' mg/dL, ';
          ...
          if (period.glucoseStats.trend !== 'stable') {
            fastingHtml += ' (' + (period.glucoseStats.rateOfChange > 0 ? '+' : '') + period.glucoseStats.rateOfChange + ' mg/dL/hr)';
          }
    ...
    if (report_plugins.basalRateOptimizer && allFastingPeriods.length > 0) {
      ...
      if (recommendationsResult && !recommendationsResult.error) {
        ...
        window.exportBasalRecommendationsToPDF = function() {
          ...
          // PDF export is marked as optional in the tasks (task 10.4)
          // For now, provide a helpful message
          alert('PDF export functionality is coming soon!\n\n' +
                'For now, you can:\n' +
                '1. Use the CSV export button to save data\n' +
                '2. Use your browser\'s Print to PDF feature (Ctrl+P or Cmd+P)\n' +
                '3. Take a screenshot of the recommendations');
          
          // Future implementation would use a library like jsPDF or pdfmake
          // Example:
          // var doc = new jsPDF();
          // doc.text('Basal Rate Recommendations', 10, 10);
          // ... add content ...
          // doc.save('basal-rate-recommendations.pdf');
        };
            ...
            context.append('rect')
              ...
              .on('mouseover', function() {
                ...
                if (period.glucoseStats) {
                  ...
                  tooltipText += 'Mean: ' + period.glucoseStats.mean + ' mg/dL<br/>';
                  tooltipText += 'Glucose Range: ' + period.glucoseStats.min + '-' + period.glucoseStats.max + ' mg/dL (span: ' + period.glucoseStats.range + ')<br/>';
                  ...
                  if (period.glucoseStats.trend !== 'stable') {
                    tooltipText += ' (' + (period.glucoseStats.rateOfChange > 0 ? '+' : '') + period.glucoseStats.rateOfChange + ' mg/dL/hr)';
                  }
