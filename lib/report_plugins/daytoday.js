...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  console.log('🔢 BUILD: FASTING-DEBUG-v2.6-CURRENT-DAY-FIX - 2026-01-28-09:20 EST');
  ...
  if (options.fasting && allFastingPeriods.length > 0) {
    ...
    // Best periods for basal testing
    var bestPeriods = allFastingPeriods.filter(function(p) { 
      return p.quality === 'excellent' || p.quality === 'good'; 
    }).slice(0, 3);
    
    if (bestPeriods.length > 0) {
      fastingHtml += '<p><b>' + translate('Best periods for basal rate analysis') + ':</b></p>';
      fastingHtml += '<ul>';
