 ...
 */
basalRateOptimizer.generatePrintHTML = function(recommendations, metadata) {
  if (!recommendations || !metadata) {
  ...
  timeBlocks.forEach(function(timeBlock) {
    ...
    if (rec.reasoning) {
      html += '<tr><td>Average Glucose Slope</td><td>' + (rec.reasoning.avgSlope !== undefined ? rec.reasoning.avgSlope + ' mg/dL/hr' : 'N/A') + '</td></tr>';
    ...
    if (rec.supportingData && rec.supportingData.length > 0) {
      ...
      html += '<table>';
      html += '<tr><th>Date</th><th>Slope (mg/dL/hr)</th><th>Duration (hours)</th><th>CV (%)</th></tr>';
      rec.supportingData.forEach(function(data) {
        html += '<tr>';
        html += '<td>' + (data.date || 'N/A') + '</td>';
        html += '<td>' + (data.slope !== undefined ? data.slope : 'N/A') + '</td>';
