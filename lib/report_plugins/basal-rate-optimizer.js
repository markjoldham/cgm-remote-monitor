...
basalRateOptimizer.applyAdjustmentLimits = function(calculatedAdjustment, currentBasalRate) {
  ...
  // Requirement 3.4: Minimum threshold of 0.025 U/hr
  var MIN_THRESHOLD = 0.025;
 ...
 * during fasting periods. Uses the formula:
 * 
 *   adjustment = (slope / ISF) * safety_multiplier
 * 
 * Where:
 * - slope: Rate of glucose change in mg/dL per hour
 * - ISF: Insulin Sensitivity Factor (mg/dL per unit)
 * - safety_multiplier: Conservative factor (0.4) to ensure cautious recommendations
 * 
 * The calculated adjustment is then:
 * 1. Limited by safety constraints (absolute and percentage limits)
 * 2. Rounded to nearest 0.05 U/hr (typical pump increment)
 * 
 * @param {number} slope - Glucose rate of change in mg/dL per hour
 * @param {number} isf - Insulin sensitivity factor (mg/dL per unit), uses default of 50 if not provided (Requirement 3.2)
 * @param {number} currentBasalRate - Current basal rate in U/hr
 * @returns {number} Recommended adjustment in U/hr (Requirement 3.5)
 */
basalRateOptimizer.calculateAdjustment = function(slope, isf, currentBasalRate) {
  try {
    // Validate inputs
    ...
    // Requirement 3.3: Use safety multiplier between 0.3 and 0.5
    // We use 0.4 as a balanced conservative factor
    var SAFETY_MULTIPLIER = 0.4;
    
    // Requirement 3.1: Apply formula: adjustment = (slope / ISF) * safety_multiplier
    var adjustment = (slope / effectiveISF) * SAFETY_MULTIPLIER;
...
basalRateOptimizer.generateRecommendation = function(timeBlock, periods, profileData) {
  try {
    ...
    // Calculate adjustment (Requirement 7.2)
    var adjustment = basalRateOptimizer.calculateAdjustment(avgSlope, isf, currentBasalRate);
