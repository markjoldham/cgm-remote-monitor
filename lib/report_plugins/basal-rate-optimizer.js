...
basalRateOptimizer.generateRecommendations = function(fastingPeriods, profileData) {
  console.log('🔢 BASAL RATE OPTIMIZER BUILD: v2.1-TDD-BASED-LIMITS - 2026-01-28-21:00 EST');
 ...
 * 
 * Requirements: 3.4, 3.6, 3.7 (modified per user preference)
 * 
 * Applies multiple safety limits to calculated basal rate adjustments:
 * 
 * 1. Absolute limit: 60% of TDD / 24 hours (user preference)
 * 2. Percentage limit: ±40% of current basal rate (user preference)
 * 3. Minimum threshold: 0.05 U/hr
 * 
 ...
 * @param {Object} profileData - Profile data with TDD (optional)
 * @returns {number} Limited adjustment in U/hr
 */
basalRateOptimizer.applyAdjustmentLimits = function(calculatedAdjustment, currentBasalRate, profileData) {
  // Validate inputs
  ...
  // Get TDD for absolute limit calculation
  var tdd = basalRateOptimizer.getTDD(profileData);
  
  // Absolute limit: 60% of TDD / 24 hours (user preference)
  // This represents a reasonable maximum hourly adjustment based on total daily needs
  var ABSOLUTE_LIMIT = (tdd * 0.60) / 24;
  console.log('💉 Absolute limit based on TDD:', ABSOLUTE_LIMIT.toFixed(3), 'U/hr (60% of', tdd.toFixed(1), 'U TDD / 24h)');
  
  if (Math.abs(limitedAdjustment) > ABSOLUTE_LIMIT) {
    console.log('⚠️ Adjustment', limitedAdjustment.toFixed(3), 'exceeds absolute limit, capping at', ABSOLUTE_LIMIT.toFixed(3));
    limitedAdjustment = Math.sign(limitedAdjustment) * ABSOLUTE_LIMIT;
  }
  
  // Percentage limit: ±40% of current rate (user preference)
  var PERCENTAGE_LIMIT = 0.40;
  var maxPercentageChange = currentBasalRate * PERCENTAGE_LIMIT;
  
  if (Math.abs(limitedAdjustment) > maxPercentageChange) {
    console.log('⚠️ Adjustment', limitedAdjustment.toFixed(3), 'exceeds 40% of current rate, capping at', maxPercentageChange.toFixed(3));
    limitedAdjustment = Math.sign(limitedAdjustment) * maxPercentageChange;
  }
  
  // Minimum threshold: 0.05 U/hr (user preference)
 ...
 */
basalRateOptimizer.calculateAdjustment = function(slope, isf, currentBasalRate, insulinDelivery, duration, profileData) {
  try {
    // Validate inputs
    ...
    // Using 0.75 for more aggressive recommendations (user preference)
    var SAFETY_MULTIPLIER = 0.75;
    ...
    adjustment = basalRateOptimizer.applyAdjustmentLimits(adjustment, currentBasalRate, profileData);
...
/**
 * Get TDD (Total Daily Dose) from profile data
 * 
 * Uses actual TDD calculated from treatment data if available,
 * otherwise estimates from basal schedule.
 * 
 * @param {Object} profileData - Profile data with optional tdd property
 * @returns {number} TDD in units per day
 */
basalRateOptimizer.getTDD = function(profileData) {
  // If TDD is explicitly provided from actual insulin delivery, use it
  if (profileData && typeof profileData.tdd === 'number' && profileData.tdd > 0) {
    console.log('💉 Using actual TDD from insulin delivery:', profileData.tdd.toFixed(1), 'U/day');
    return profileData.tdd;
  }
  
  // Otherwise, estimate from basal schedule (basal is typically 40-50% of TDD)
  if (profileData && profileData.basalSchedule && Array.isArray(profileData.basalSchedule)) {
    var totalBasal = 0;
    
    for (var i = 0; i < profileData.basalSchedule.length; i++) {
      var segment = profileData.basalSchedule[i];
      var nextSegment = profileData.basalSchedule[i + 1];
      
      // Parse start time
      var startParts = segment.time.split(':');
      var startHour = parseInt(startParts[0], 10);
      
      // Calculate duration until next segment (or end of day)
      var endHour = nextSegment ? parseInt(nextSegment.time.split(':')[0], 10) : 24;
      if (endHour < startHour) endHour += 24; // Handle midnight rollover
      
      var duration = endHour - startHour;
      totalBasal += segment.value * duration;
    }
    
    // Estimate TDD as 2.2x total basal (assumes basal is ~45% of TDD)
    var estimatedTDD = totalBasal * 2.2;
    console.log('💉 Estimated TDD from basal schedule:', estimatedTDD.toFixed(1), 'U/day');
    return estimatedTDD;
  }
  
  // Default fallback: assume 40 units/day
  console.log('💉 Using default TDD: 40 U/day');
  return 40;
};
...
basalRateOptimizer.generateRecommendation = function(timeBlock, periods, profileData) {
  try {
    ...
    var adjustment = basalRateOptimizer.calculateAdjustment(avgSlope, isf, currentBasalRate, avgInsulinDelivery, avgDuration, profileData);
...
basalRateOptimizer.formatSupportingData = function(supportingData, units) {
  ...
  supportingData.forEach(function(data) {
    var displaySlope = data.slope;
    if (typeof displaySlope === 'number' && units === 'mmol') {
      displaySlope = Math.round((displaySlope / 18) * 100) / 100;
    }
    
    html += '<tr>';
    html += '<td>' + (data.date || 'N/A') + '</td>';
    html += '<td>' + (typeof displaySlope === 'number' ? displaySlope.toFixed(2) : 'N/A') + '</td>';
 ...
 */
basalRateOptimizer.generateTimeBlockHTML = function(recommendation, units) {
  if (!recommendation) {
    return '';
  }
  ...
  if (recommendation.reasoning) {
    var displaySlope = recommendation.reasoning.avgSlope || 0;
    if (units === 'mmol') {
      displaySlope = Math.round((displaySlope / 18) * 100) / 100;
    }
    ...
    html += 'Average slope: ' + displaySlope + ' ' + unitsLabel + '<br>';
  ...
  if (recommendation.supportingData && recommendation.supportingData.length > 0) {
    ...
    html += basalRateOptimizer.formatSupportingData(recommendation.supportingData, units);
    html += '</div>';
  }
 ...
 * @param {Object} profileData - Profile data with units (optional)
 * @returns {string} HTML string for the complete recommendations section
 */
basalRateOptimizer.generateRecommendationsHTML = function(recommendations, metadata, profileData) {
  if (!recommendations || !metadata) {
    return '<div class="basal-rate-recommendations"><p>No recommendations available.</p></div>';
  }
  
  // Get units from profileData, default to mg/dl
  var units = (profileData && profileData.units) || 'mg/dl';
  ...
  timeBlocks.forEach(function(timeBlock) {
    if (recommendations[timeBlock]) {
      html += basalRateOptimizer.generateTimeBlockHTML(recommendations[timeBlock], units);
    }
  });
 ...
 */
basalRateOptimizer.exportToCSV = function(recommendations, metadata, profileData) {
  if (!recommendations || !metadata) {
  ...
  timeBlocks.forEach(function(timeBlock) {
    ...
    if (rec.adjustment !== undefined && rec.adjustment !== null) {
      if (rec.adjustment === 0) {
        adjustmentStr = '0.00';
      } else {
    ...
    // Reasoning fields
    if (rec.reasoning) {
      var displaySlope = rec.reasoning.avgSlope;
      if (displaySlope !== undefined && units === 'mmol') {
        displaySlope = Math.round((displaySlope / 18) * 100) / 100;
      }
      row.push(escapeCSV(displaySlope !== undefined ? displaySlope : 'N/A'));
  ...
  csv.push('Time Block' + DELIMITER + 'Date' + DELIMITER + 'Slope (' + unitsLabel + ')' + DELIMITER + 'Duration (hours)' + DELIMITER + 'CV (%)');
  ...
  timeBlocks.forEach(function(timeBlock) {
    ...
    rec.supportingData.forEach(function(data) {
      ...
      var supportingRow = [
        ...
        escapeCSV(displaySlope !== undefined ? displaySlope : 'N/A'),
 ...
 * Requirement: 9.1
 * 
 * Creates a complete HTML document optimized for printing/PDF export
 ...
 */
basalRateOptimizer.generatePrintHTML = function(recommendations, metadata, profileData) {
  if (!recommendations || !metadata) {
    return '<html><body><h1>Error: No recommendations available</h1></body></html>';
  }
  ...
  html += '<title>Basal Rate Optimization Recommendations</title>';
  html += '<style>';
  html += 'body { font-family: Arial, sans-serif; margin: 20px; color: #333; }';
  html += 'h1 { color: #0066cc; border-bottom: 3px solid #0066cc; padding-bottom: 10px; }';
  html += 'h2 { color: #0066cc; margin-top: 30px; page-break-after: avoid; }';
  html += 'h3 { color: #666; margin-top: 20px; }';
  html += '.disclaimer { background-color: #fff3cd; border: 2px solid #ffc107; padding: 15px; margin: 20px 0; page-break-inside: avoid; }';
  html += '.disclaimer strong { color: #856404; }';
  html += '.metadata { background-color: #e7f3ff; padding: 15px; margin: 20px 0; border-left: 4px solid #0066cc; page-break-inside: avoid; }';
  html += '.time-block { margin: 20px 0; padding: 15px; border: 1px solid #ddd; page-break-inside: avoid; }';
  html += '.time-block.high { border-left: 5px solid #28a745; background-color: #f0fff4; }';
  html += '.time-block.medium { border-left: 5px solid #ffc107; background-color: #fffbf0; }';
  html += '.time-block.low { border-left: 5px solid #dc3545; background-color: #fff5f5; }';
  html += '.recommendation { font-size: 18px; font-weight: bold; margin: 10px 0; }';
  html += '.recommendation.increase { color: #dc3545; }';
  html += '.recommendation.decrease { color: #28a745; }';
  html += '.recommendation.no-change { color: #666; }';
  ...
  html += '.warning { background-color: #fff3cd; padding: 10px; margin: 10px 0; border-left: 4px solid #ffc107; }';
  html += '.safety-tips { background-color: #f0f8ff; padding: 15px; margin: 20px 0; page-break-inside: avoid; }';
  html += '.safety-tips li { margin: 8px 0; }';
  html += '@media print { body { margin: 15mm; } .page-break { page-break-before: always; } }';
  html += '</style></head><body>';
  
  // Title
  html += '<h1>Basal Rate Optimization Recommendations</h1>';
  ...
  html += '<p><strong>Date Range:</strong> ' + (metadata.startDate || 'N/A') + ' to ' + (metadata.endDate || 'N/A') + '</p>';
  html += '<p><strong>Total Fasting Periods Analyzed:</strong> ' + (metadata.periodCount || 0) + '</p>';
  html += '<p><strong>Analysis Generated:</strong> ' + (metadata.analysisDate || 'N/A') + '</p>';
  html += '</div>';
  
  // Recommendations by time block
  html += '<h2>Recommendations by Time of Day</h2>';
  ...
  timeBlocks.forEach(function(timeBlock) {
    ...
    var confidenceClass = rec.confidenceLevel || 'low';
    html += '<div class="time-block ' + confidenceClass + '">';
    html += '<h3>' + (rec.timeBlockLabel || basalRateOptimizer.getTimeBlockLabel(timeBlock)) + '</h3>';
    
    // Recommendation
    var recClass = 'no-change';
    if (rec.adjustment > 0) recClass = 'increase';
    if (rec.adjustment < 0) recClass = 'decrease';
    
    html += '<div class="recommendation ' + recClass + '">';
    html += rec.recommendation || 'NO CHANGE';
    html += '</div>';
    
    // Details table
    html += '<table>';
    html += '<tr><th>Metric</th><th>Value</th></tr>';
    html += '<tr><td>Current Basal Rate</td><td>' + (rec.currentBasalRate !== undefined ? rec.currentBasalRate.toFixed(2) + ' U/hr' : 'N/A') + '</td></tr>';
    
    if (rec.adjustment !== undefined && rec.adjustment !== 0) {
      var sign = rec.adjustment > 0 ? '+' : '';
      html += '<tr><td>Adjustment</td><td>' + sign + rec.adjustment.toFixed(2) + ' U/hr</td></tr>';
      html += '<tr><td>New Basal Rate</td><td>' + (rec.newBasalRate !== undefined ? rec.newBasalRate.toFixed(2) + ' U/hr' : 'N/A') + '</td></tr>';
    }
    
    html += '<tr><td>Confidence Score</td><td>' + (rec.confidenceScore !== undefined ? rec.confidenceScore + '/100' : 'N/A') + '</td></tr>';
    html += '<tr><td>Confidence Level</td><td>' + (rec.confidenceLevel || 'N/A').toUpperCase() + '</td></tr>';
    ...
    if (rec.reasoning) {
      ...
      html += '<tr><td>Average Glucose Slope</td><td>' + (displaySlope !== undefined ? displaySlope + ' ' + unitsLabel : 'N/A') + '</td></tr>';
      html += '<tr><td>Average CV%</td><td>' + (rec.reasoning.avgCV !== undefined ? rec.reasoning.avgCV + '%' : 'N/A') + '</td></tr>';
      html += '<tr><td>Qualifying Periods</td><td>' + (rec.reasoning.qualifyingPeriods || 0) + ' of ' + (rec.reasoning.totalPeriods || 0) + '</td></tr>';
    }
    
    html += '</table>';
    
    // Supporting data
    if (rec.supportingData && rec.supportingData.length > 0) {
      html += '<h4>Supporting Data</h4>';
      html += '<table>';
      html += '<tr><th>Date</th><th>Slope (' + unitsLabel + ')</th><th>Duration (hours)</th><th>CV (%)</th></tr>';
      rec.supportingData.forEach(function(data) {
        var displaySlope = data.slope;
        if (displaySlope !== undefined && units === 'mmol') {
          displaySlope = Math.round((displaySlope / 18) * 100) / 100;
        ...
        html += '<tr>';
        html += '<td>' + (data.date || 'N/A') + '</td>';
        html += '<td>' + (displaySlope !== undefined ? displaySlope : 'N/A') + '</td>';
        html += '<td>' + (data.duration !== undefined ? data.duration : 'N/A') + '</td>';
        html += '<td>' + (data.cv !== undefined ? data.cv : 'N/A') + '</td>';
        html += '</tr>';
      });
      html += '</table>';
    }
    ...
    if (rec.warnings && rec.warnings.length > 0) {
      rec.warnings.forEach(function(warning) {
  ...
  html += '<div class="safety-tips">';
  html += '<h2>Safety Tips for Implementing Changes</h2>';
  html += '<ol>';
  html += '<li><strong>Test one time period at a time</strong> - Only adjust one basal rate segment at a time to clearly see the effect</li>';
  html += '<li><strong>Wait 2-3 days between adjustments</strong> - Give your body time to adjust and collect enough data</li>';
  html += '<li><strong>Monitor for hypoglycemia</strong> - Watch closely for low blood sugar, especially after rate increases</li>';
  html += '<li><strong>Keep detailed notes</strong> - Document all changes and observed effects in your diabetes management app</li>';
  html += '<li><strong>Start with high-confidence recommendations</strong> - Prioritize adjustments with high confidence scores (70+)</li>';
  html += '<li><strong>Be conservative</strong> - When in doubt, make smaller adjustments than recommended</li>';
  html += '<li><strong>Consult your healthcare team</strong> - Review these recommendations with your doctor or diabetes educator</li>';
  html += '</ol>';
  html += '</div>';
