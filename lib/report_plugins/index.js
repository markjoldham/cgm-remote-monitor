...
function init(ctx) {
  ...
  // Load fasting detection utility
  console.log('🔧 Loading fasting detection module...');
  console.log('🔢 REPORT PLUGINS BUILD: FASTING-DEBUG-v3.7-FIX-TARGET-RANGE - 2026-01-28-12:30 EST');
  try {
    plugins.fastingDetection = require('./fasting-detection-debug'); // Use debug version
    console.log('✅ Loaded fasting detection debug module successfully');
    console.log('Fasting detection module:', plugins.fastingDetection);
  } catch (e) {
    console.error('❌ Fasting detection loading failed:', e);
    plugins.fastingDetection = null;
  }

  // Load basal rate optimizer utility
  console.log('🔧 Loading basal rate optimizer module...');
  try {
    plugins.basalRateOptimizer = require('./basal-rate-optimizer')();
    console.log('✅ Loaded basal rate optimizer module successfully');
    console.log('Basal rate optimizer module:', plugins.basalRateOptimizer);
  } catch (e) {
    console.error('❌ Basal rate optimizer loading failed:', e);
    plugins.basalRateOptimizer = null;
  }
